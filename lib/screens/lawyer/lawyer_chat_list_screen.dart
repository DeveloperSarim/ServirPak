...
class _LawyerChatListScreenState extends State<LawyerChatListScreen> {
  ...
  Future<void> _loadCurrentLawyerId() async {
    try {
      ...
      setState(() {
        _chatsStream = RealtimeChatService.listenToLawyerChatUpdates(
          _currentLawyerId!,
        );
      });
    } catch (e) {
  ...
  Widget _buildChatCard(ChatModel chat) {
    ...
    bool hasUnreadMessages = chat.lastMessageSenderId != _currentLawyerId;
              ...
              Stack(
                children: [
                  CircleAvatar(
                    ...
                    backgroundImage: chat.userProfileImage != null
                        ? NetworkImage(chat.userProfileImage!)
                        : null,
                    child: chat.userProfileImage == null
                        ? const Icon(
                            Icons.person,
                            color: Color(0xFF8B4513),
                            size: 32,
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        if (chat.lastMessageSenderId == _currentLawyerId)
                          const Icon(
                            Icons.check,
                            size: 16,
                            color: Colors.green,
                  ...
                  PopupMenuItem(
                    ...
                    child: Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Text(
                          chat.lawyerHasBlocked ? 'Unblock User' : 'Block User',
                        ),
                      ],
                    ),
                  ),
  ...
  Future<void> _toggleBlock(ChatModel chat) async {
    try {
      await ChatService.toggleUserBlock(
        ...
        isLawyer: true,
        blockStatus: !chat.lawyerHasBlocked,
      );

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            chat.lawyerHasBlocked ? 'User unblocked' : 'User blocked',
          ),
          backgroundColor: chat.lawyerHasBlocked ? Colors.green : Colors.red,
        ),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _showUserProfile(ChatModel chat) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
